...
function route($route, $path_to_include){
  ...
  $dir= dirname(__DIR__);
  if($route == "/404"){
    include_once $dir."/app/$path_to_include";
    exit();
  }
...
$url=str_replace($dirs[2],'',$_SERVER['REQUEST_URI']);
$urls=str_replace('//','/',$url);
//echo $urls;  
  $request_url = filter_var($urls, FILTER_SANITIZE_URL);
  ...
  if( $route_parts[0] == '' && count($request_url_parts) == 0 ){
    ...
    //die();
    include_once $dir."/$path_to_include";
    exit();
  }
  ...
  function renderview($path_to_include)
{
